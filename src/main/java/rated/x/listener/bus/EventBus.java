...
{
    // the event bus logger, separate from the Mebula logger
    ...
    public boolean dispatch(Object event)
    {
        ...
        if (subscribers != null && !subscribers.isEmpty())
        {
            ...
            for (Subscriber subscriber : subscribers)
            {
                ...
                if (EventCancelable.class.isAssignableFrom(event.getClass().getSuperclass()))
                {
                    canceled = ((EventCancelable) event).isCanceled();
                }
            }
